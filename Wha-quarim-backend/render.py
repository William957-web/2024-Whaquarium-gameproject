...
def reg(username, pwd):
    ...
    if cur:
        ...
        cur = db.execute("INSERT INTO users (username, password, fishes, foods, money, play, pre_fishes, achieve, token, level) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);", (username, sha256(pwd.encode()).hexdigest(), '{}', '{}', 100, 0, '{}', '{}', token, 1))
        db.commit() 
        return token
